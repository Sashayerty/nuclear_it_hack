...
import { loginThunk, setAuthModalMode, clearError } from '../../store/slices/authSlice'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (loginThunk.fulfilled.match(resultAction)) {
      dispatch(
        addNotification({
          type: 'success',
          message: `Добро пожаловать, ${resultAction.payload.name}!`,
        })
      )
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="w-full bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white font-medium py-2.5 px-4 rounded-lg shadow-md hover:shadow-sky-500/20 transition-all flex items-center justify-center space-x-2 disabled:opacity-50"
      >
        {isLoading ? (
