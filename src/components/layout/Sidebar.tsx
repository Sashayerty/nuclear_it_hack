...
export const Sidebar = () => {
  ...
  const navItems: { id: ActiveTab; label: string; iconKey: string }[] = [
    ...
    ...(auth.role === 'admin' ? [{ id: 'admin' as ActiveTab, label: 'Панель администратора', iconKey: 'admin' }] : [])
  ]
  ...
  return (
    <aside className="w-16 md:w-20 bg-[#18191c] text-slate-400 flex flex-col items-center justify-between py-6 shrink-0 border-r border-slate-800/60 sticky top-0 h-screen select-none">
      ...
      <div className="flex flex-col items-center gap-3 w-full px-2">
        <button
          onClick={() => dispatch(setActiveTab('auth'))}
          title="Сменить профиль"
          className="h-10 w-10 rounded-xl text-slate-500 hover:text-slate-200 hover:bg-slate-800/50 flex items-center justify-center transition-colors"
        >
          {ICONS.auth(false)}
        </button>
        <button
